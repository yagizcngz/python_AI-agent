...
•	OpenAI: OpenRouter sunucularına bağlanıp yapay zeka ile mesajlaşmamızı sağlayan ana köprü kütüphanesidir.
...
•	Burada yapay zekaya "Senin ellerin ve gözlerin var, bunları kullanabilirsin" diyerek araçların kullanım kılavuzlarını (JSON formatında) veriyoruz. Model bu açıklamalara bakarak hangi aracı ne zaman seçeceğine karar verir.
...
•	Ajan mevcut messages (hafıza) ve tools (araçlar) ile OpenRouter'a istek atar ve yapay zekadan "Ne yapmalıyım?" diye cevap bekler.
...
stderr kullandığın için: İşletim sistemi stderr mesajlarını dosyaya yönlendirmez. Bu loglar sadece senin ekranında akar (böylece asistanın
...
Eğer prompt yazarken kullanılması gereken aracı belirtmediysen dosya varsayılan olarak utf-16 formatında kaydeder. \xff ibaresi bu formatın kullanıldığı anlamına gelir.
...
formatı zorla utf-8 olarak yazdıran | Out-File -Encoding utf8 ekleybilirsin ya da Write aracını kullanabilirsin çünkü içerisinde dosya formatı utf-8 var.
...
**Error Code 429 hatası nedir ?**
1- kullandığınız yapay zeka kısa süreliğine fazla isteğe maruz kaldığından dolayı cevap veremiyor (... :free is temporarily rate-limited upstream. Please retry shortly:). 
Kısa süre içerisinde tekrar denerseniz isteğiniz gerçekleşebilir. Ya da farklı model kullanabilirsiniz. 
...
**Neden import openai from OpenAI yapıyoruz ?**
...
**Neden Kütüphane Olarak openai Kullanıyoruz?**
...
Bunun yerine bilgisayarımıza indirdiğimiz o openai Python kütüphanesini kullanıyoruz. Bu kütüphane, senin promptlarını ve araçlarını (paketleri) OpenAI standartlarında paketliyor.
...
Eğer programı Command Promt veya PowerShell üzerinden kullanmaya çalışırsanız Ajan arka planda systeminfo gibi bir Windows komutu çalıştırdığında Windows, bu komutun sonucunu terminale basarken Türkçe karakterler (ı, ş, ğ) veya sisteme özel bazı semboller kullanıyor. Python da terminalden gelen bu metni okumaya çalışırken karakterleri tanıyamadığı için UnicodeDecodeError hatasını veriyor. errors="replace" parametresi Python okuyamadığı garip bir sembol veya TÜrkçe karakter ile karşılaştığında programı çökertmek yerine ? kullanarak sorunsuz bir şekilde devam etmesini sağlar.